...
export default {
  ...
  mounted () {
    console.log('init client content6 mounted begin')
    eventCenter.$on('init-content', inputInit => {
      if (inputInit.updateUrl === '/api/setClientConfig6') {
        this.updateUrl = inputInit.updateUrl
        this.nodeTableContent = inputInit.initContent.nodes
      }
      console.log('listen event of init client content6: ' + this.initContent)
    })
  },
  methods: {
    onSubmit () {
