  ...
  data () {
    return {
      ...
      nodeTableContent: [
        ...
        { selected: false, id: 'HX1F_S8', name: 'default', titles: '招商水印', exeFile: 'default', remark: 'HX视频' }
      ],
      tableContent: { nodes: [] }
    }
  },
  mounted () {
    console.log('init client content5 mounted begin')
    eventCenter.$on('init-content', inputInit => {
      if (inputInit.updateUrl === '/api/setClientConfig5') {
        this.updateUrl = inputInit.updateUrl
        this.nodeTableContent = inputInit.initContent.nodes
      }
      console.log('listen event of init client content5: ' + this.initContent)
    })
  },
  methods: {
    onSubmit () {
