...
export default {
  ...
  mounted () {
    console.log('init client content7 mounted begin')
    eventCenter.$on('init-content', inputInit => {
      if (inputInit.updateUrl === '/api/setClientConfig7') {
        this.updateUrl = inputInit.updateUrl
        this.nodeTableContent = inputInit.initContent.nodes
      }
      console.log('listen event of init client content7: ' + this.initContent)
    })
  },
  methods: {
    onSubmit () {
