...
export default {
  ...
  mounted () {
    console.log('init client content4 mounted begin')
    eventCenter.$on('init-content', inputInit => {
      if (inputInit.updateUrl === '/api/setClientConfig4') {
        this.updateUrl = inputInit.updateUrl
        this.nodeTableContent = inputInit.initContent.nodes
      }
      console.log('listen event of init client content4: ' + this.initContent)
    })
  },
  methods: {
    onSubmit () {
